    ...
    <div className="flex flex-wrap justify-center gap-5 max-w-[1680px] mx-auto">
      {
        items.map((nData: any) => {
          ...
          // console.log(nData)
          return (
            <Link href={`youtube/${nData.id}`} key={nData.id} className={`min-w-full sm:min-w-[360px] bg-linear-to-b from-black/0 via-black/10 to-white/5 hover:to-teal-300/20 p-1 sm:rounded-2xl shadow-xs shadow-stone-500 hover:shadow-md hover:shadow-slate-300/50 `} style={{ width: nData.snippet.thumbnails.medium.width }} >
              <Suspense fallback={'🔘'}>
